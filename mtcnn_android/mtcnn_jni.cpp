...
Java_faceDetector_MTCNN_FaceDetectionModelInit(JNIEnv *env, jobject instance,
    ...
    LOGD("模型文件路径： %s", tFaceModelDir.c_str());
...
Java_faceDetector_MTCNN_FaceDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
                                    jint imageWidth, jint imageHeight, jint imageChannel, jfloat resizeParam) {
    //  LOGD("JNI开始检测人脸");
    if(!detection_sdk_init_ok){
    ...
    ncnn::Mat ncnn_img;
    if(imageChannel==3) {
       ncnn_img = ncnn::Mat::from_pixels_resize(faceImageCharDate, ncnn::Mat::PIXEL_BGR2RGB,
                                                    imageWidth, imageHeight,imageWidth/resizeParam,imageHeight/resizeParam);
        LOGD("resize后数据宽=%d,高=%d,通道=%d",imageWidth/resizeParam,imageHeight/resizeParam,imageChannel);
    }else{
    ...
    for(int i=0;i<num_face;i++){
        faceInfo[14*i+1] = finalBbox[i].x1*resizeParam;//left
        faceInfo[14*i+2] = finalBbox[i].y1*resizeParam;//top
        faceInfo[14*i+3] = finalBbox[i].x2*resizeParam;//right
        faceInfo[14*i+4] = finalBbox[i].y2*resizeParam;//bottom
        for (int j =0;j<10;j++){
            faceInfo[14*i+5+j]=static_cast<int>(finalBbox[i].ppoint[j]*resizeParam);
        }
    }
...
JNIEXPORT jintArray JNICALL
Java_faceDetector_MTCNN_MaxFaceDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
                                       jint imageWidth, jint imageHeight, jint imageChannel,jfloat resizeParam) {
    //  LOGD("JNI开始检测人脸");
    if(!detection_sdk_init_ok){
        LOGD("人脸检测MTCNN模型SDK未初始化，直接返回空");
...
JNIEXPORT jboolean JNICALL
Java_faceDetector_MTCNN_FaceDetectionModelUnInit(JNIEnv *env, jobject instance) {
    if(!detection_sdk_init_ok){
...
JNIEXPORT jboolean JNICALL
Java_faceDetector_MTCNN_SetMinFaceSize(JNIEnv *env, jobject instance, jint minSize) {
    if(!detection_sdk_init_ok){
...
JNIEXPORT jboolean JNICALL
Java_faceDetector_MTCNN_SetThreadsNumber(JNIEnv *env, jobject instance, jint threadsNumber) {
    if(!detection_sdk_init_ok){
        LOGD("人脸检测MTCNN模型SDK未初始化，直接返回");
        return false;
    }
